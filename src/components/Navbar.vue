...
<template>
	<Disclosure as="nav" class="bg-gradient-to-t from-teal-500 to-teal-400" v-slot="{ open }">
		<div class="max-w-full mx-auto px-2 sm:px-6 lg:px-4">
			<div class="relative flex items-center justify-between h-14">
				<div class="absolute inset-y-0 left-0 flex items-center sm:hidden">
					<!-- Mobile menu button-->
					<DisclosureButton class="inline-flex items-center justify-center p-2 rounded-md text-white hover:bg-black hover:bg-opacity-10 hover:shadow-inner focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white">
				...
				<div class="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
					<div class="group flex-shrink-0 flex items-center">
						...
						<span class="text-white text-xl font-medium tracking-wide text-shadow">
							brello
						</span>
					</div>
					<div class="hidden sm:block sm:ml-6">
						<div class="flex space-x-3">
							...
							>
								<NavLink :class="[isActive ? 'bg-white bg-opacity-20 text-white' : 'text-white text-opacity-70 hover:text-opacity-100 text-shadow', 'px-3 py-2 rounded text-sm font-medium']" :href="href" @click="navigate">
									{{ item.name }}
								</NavLink>
							</router-link>
						</div>
				...
				<div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
					<!-- Twitter share -->
					<a href="https://twitter.com/intent/tweet?url=https%3A%2F%2Fbrello-board.vercel.app&text=Check%20out%20Brello%2C%20a%20local-storage%20kanban%20board&hashtags=brello%2Cmadewithvue%2Ctailwindcss" class="flex items-center space-x-2 text-white hover:text-gray-200 transition-colors duration-150 text-sm">
						<!-- Twitter icon -->
					...
					<a href="https://classic.heroicons.com" class="hidden group text-white text-opacity-70 text-shadow text-sm ml-10">
						Want to contribute?
